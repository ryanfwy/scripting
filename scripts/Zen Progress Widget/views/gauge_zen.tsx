import { Gauge, Image, Path, Script, SVG, Text } from "scripting"
import { keyZenCnt, fontWidgetLabel } from "../components/constant"
import { getSetting } from "../components/setting"

const imageRatio = 267 / 200
const imageSize = 23

const zenMaxCnt = getSetting("zenMaxCnt")
const zenRoundMaxCnt = getSetting("zenRoundMaxCnt")

function padZeros(num: number) {
  const digit = zenMaxCnt.toString().length
  return num.toString().padStart(digit, '0')
}

export function GaugeZen() {
  let cnt = Storage.get<number>(keyZenCnt) || 0
  if (cnt > zenMaxCnt) {
    cnt = 0
    Storage.set(keyZenCnt, cnt)
  }
